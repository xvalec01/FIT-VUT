...
**/
	if (*RootPtr != NULL) 
	{
		if ((*RootPtr)->Key == K) 
		{
			if (((*RootPtr)->RPtr != NULL) && ((*RootPtr)->LPtr != NULL)) // levy i pravy podstrom
			{
				ReplaceByRightmost((*RootPtr), &(*RootPtr)->LPtr);
			} 
			else if (((*RootPtr)->LPtr != NULL) || ((*RootPtr)->RPtr != NULL)) // ma jeden podstrom
			{
				if ((*RootPtr)->RPtr != NULL) // prava vetev
				{
		...
		else 	// nenalezl klic, pokracuje v dalsi vetvi
		{
			if (K < (*RootPtr)->Key) 
